...
public class Weather {
	...
	public static ImmutableMap<Integer, Double> getWeather(String loc) throws Exception {
			...
			if (df.isValid()) {
				for (int i = 0; i < df.getForecastCount(); i++) {
					...
					if (forecast.hasDateTime()) {
						multis.put(i + 1,
								weatherMulti(forecast.getTemperatureInstance().getDayTemperature(), forecast.getRain(),
										forecast.getSnow(), forecast.getHumidity(), forecast.getWindSpeed()));
					}
				}
			}
		}
	...
	public static Forecast getWeather(String location, Date date) throws Exception {
		GeoApiContext context = new GeoApiContext().setApiKey("AIzaSyCLY-qNiBx8jDzJAzLU1S8tewokC6BKQ_M");
		float lat = Float.NaN;
		float lng = Float.NaN;
		...
		if (lat != Float.NaN && lng != Float.NaN) {
			...
			if (df.isValid()) {
				for (int i = 0; i < df.getForecastCount(); i++) {
					DailyForecast.Forecast forecast = df.getForecastInstance(i);
					System.out.println("datetime: " + forecast.getDateTime() + " | date: " + date);
	...
	private static double weatherMulti(float temp, float rain, float snow, float humid, float wind) {
		...
		if (humid != Float.NaN) {
			// If you want to look at humidity
		}

		if (wind != Float.NaN) {
			// If you want to look at wind
		}
