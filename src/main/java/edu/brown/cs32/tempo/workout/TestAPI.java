...
public class TestAPI {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Group group = null; // new Group(null, null, null, new Team(null, "02901",
                        // null, null, null));
    GeoApiContext context = new GeoApiContext()
        .setApiKey("AIzaSyCLY-qNiBx8jDzJAzLU1S8tewokC6BKQ_M");
    GeocodingResult[] results = GeocodingApi
        .geocode(context, group.getTeam().getLocation().getPostalCode())
        .await();
    ...
    CurrentWeather cwd = owm.currentWeatherByCoordinates(
        (float) results[0].geometry.location.lat,
    ...
    if (cwd.getMainInstance() != null) {
      ...
      System.out
          .println("Temperature: " + cwd.getMainInstance().getMaxTemperature()
              + "/" + cwd.getMainInstance().getMinTemperature() + "\'F");
      System.out.println("Weather:" + owm
    ...
    if (!df.isValid()) {
      ...
      for (int i = 0; i < df.getForecastCount(); i++) {
        ...
        if (forecast.hasDateTime()) {
          ...
          System.out
              .println(forecast.getTemperatureInstance().getDayTemperature());
